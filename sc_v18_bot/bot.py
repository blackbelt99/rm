# ╔══════════════════════════════════════════════════════════════╗
# ║         STRENGTHCLOUD BOT — Python Edition (discord.py)      ║
...
TICKET_PING_ROLE_ID = _int_env("SUPPORT_ROLE_ID")  # role to ping on ticket create (changeable with $ticket setping)
...
def ok_embed(msg: str) -> discord.Embed:
    return discord.Embed(description=f"<:yes:✅>  {msg}", color=0x57f287)
...
class TitleDescModal(discord.ui.Modal, title="Edit Title & Description"):
    panel_title = discord.ui.TextInput(
        label="Panel Title",
        placeholder="StrengthCloud — Support Ticket",
        max_length=100
    )
...
class HoursFooterModal(discord.ui.Modal, title="Edit Hours & Footer"):
    ...
    footer = discord.ui.TextInput(
        label="Footer Text",
        placeholder="StrengthCloud | strengthcloud.net",
        max_length=80,
        required=False
    )
...
class PingRoleModal(discord.ui.Modal, title="Set Ping Role"):
    role_id = discord.ui.TextInput(
        label="Role ID to ping on ticket create",
        placeholder="Right-click role → Copy ID (e.g. 123456789012345678)",
        max_length=30,
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not val:
            ...
            await interaction.response.send_message(embed=ok_embed("Ping role removed. No role will be pinged."), ephemeral=True)
            return
        try:
            rid = int(val)
            role = interaction.guild.get_role(rid)
            if not role:
                return await interaction.response.send_message(embed=err_embed("Role not found. Make sure the ID is correct."), ephemeral=True)
            TICKET_PING_ROLE_ID = rid
            await interaction.response.send_message(embed=ok_embed(f"Ping role set to {role.mention}\nThis role will be pinged when a ticket is created."), ephemeral=True)
        except ValueError:
            await interaction.response.send_message(embed=err_embed("Invalid ID. Please enter a valid Role ID."), ephemeral=True)
...
class TicketSetupView(discord.ui.View):
    ...
    async def set_ping(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = PingRoleModal()
        cur = TICKET_PING_ROLE_ID
        modal.role_id.default = str(cur) if cur else ""
        await interaction.response.send_modal(modal)
...
PANEL_CONFIG = {
    "title":       "StrengthCloud — Support Ticket",
    "description": (
    ...
    "footer":        "StrengthCloud | strengthcloud.net",
    "color":         0x5865f2,
}
...
class TicketCategorySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        e.set_footer(text="StrengthCloud Support • $ticket close to close • Made by Black Belt")
        ...
        if TICKET_PING_ROLE_ID:
            ...
            ping_content = f"<@&{SUPPORT_ROLE_ID}>"
        ...
        if TICKET_LOG_ID:
            ...
            if log_ch:
                le = discord.Embed(title="🎫 New Ticket Opened", color=C_TICKET, timestamp=datetime.now(timezone.utc))
                le.add_field(name="User",     value=f"{user} ({user.id})", inline=True)
                le.add_field(name="Category", value=label,                 inline=True)
...
class CloseTicketView(discord.ui.View):
    ...
    async def claim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await ch.edit(overwrites=new_overwrites, topic=f"{ch.topic or ""} | Claimed by {member}")
...
class ConfirmCloseView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if TICKET_LOG_ID:
            ...
            if log_ch:
                le = discord.Embed(title="🔒 Ticket Closed", color=C_ERROR, timestamp=datetime.now(timezone.utc))
...
# ── Status rotation ───────────────────────────────────────────────────────────
STATUSES = [
    (discord.ActivityType.watching,  "StrengthCloud | $help"),
    (discord.ActivityType.listening, "$help for commands"),
    (discord.ActivityType.watching,  f"tickets & invites"),
    (discord.ActivityType.playing,   "Made by Black Belt"),
]
...
async def rotate_status():
    ...
    idx = 0
    while not bot.is_closed():
        atype, aname = STATUSES[idx % len(STATUSES)]
...
async def on_ready():
    ...
    await bot.change_presence(
        status=discord.Status.online,
        activity=discord.Activity(type=discord.ActivityType.watching, name="StrengthCloud | $help"))
...
async def on_member_remove(member):
    ...
    if iid:
        ...
        # Left count always goes up
        data["left"] += 1
        # If it was a real invite, remove from invites count
        # So total invites stays accurate (only people still in server)
        if mtype == "real":
            data["invites"] = max(0, data["invites"] - 1)
...
async def ticket_rename(interaction: discord.Interaction, name: str):
    ...
    await interaction.response.send_message(embed=ok_embed(f"Renamed to `ticket-{clean}`."))
...
async def botinfo_cmd(interaction: discord.Interaction):
    e = discord.Embed(
        title="🤖 StrengthCloud Bot",
        description="A full-featured Discord bot for StrengthCloud community management.",
...
async def help_cmd(ctx, category: str = None):
    ...
    if category is None:
        e = discord.Embed(
            title="\U0001f4d6 StrengthCloud Bot \u2014 All Commands",
            description=f"Prefix: `$` | Use `$help <category>` for more details\nAll commands also work as `/slash` commands!",
            color=C_PRIMARY, timestamp=datetime.now(timezone.utc)
        )
        e.add_field(name="\U0001f3ab  Ticket System",
            value=f"`{P}ticket panel` \u2014 Send ticket panel\n`{P}ticket setup` \u2014 Customize panel\n`{P}ticket addcat <emoji> <name>` \u2014 Add category\n`{P}ticket removecat <name>` \u2014 Remove category\n`{P}ticket listcats` \u2014 List categories\n`{P}ticket close` \u2014 Close ticket",
            inline=False)
        e.add_field(name="\U0001f6e1\ufe0f  Moderation",
            value=f"`{P}timeout @u 10m` \u2014 Timeout\n`{P}untimeout @u` \u2014 Remove timeout\n`{P}kick @u [r]` \u2014 Kick\n`{P}ban @u [r]` \u2014 Ban\n`{P}unban <id>` \u2014 Unban\n`{P}warn @u <r>` \u2014 Warn (DMs user)\n`{P}warnings @u` \u2014 View warns\n`{P}clearwarns @u` \u2014 Clear warns\n`{P}purge <n>` \u2014 Delete messages\n`{P}lock` / `{P}unlock` \u2014 Lock channel\n`{P}slowmode <s>` \u2014 Slowmode\n`{P}note @u [text]` \u2014 Staff notes",
            inline=False)
        e.add_field(name="\U0001f4a4  AFK System",
            value=f"`{P}afk [reason]` \u2014 Set AFK status\n> Adds [AFK] to nick, auto-replies on ping, auto-removes on message",
            inline=False)
        e.add_field(name="\U0001f4e8  Invite Tracking",
            value=f"`{P}i [@user]` \u2014 Invite stats\n`{P}invited [@user]` \u2014 Invited list\n`{P}lb` \u2014 Invite leaderboard\n`{P}resetinvites @u` \u2014 Reset invites (Admin)",
            inline=False)
        e.add_field(name="\U0001f527  Utility",
            value=f"`{P}userinfo [@u]` \u2014 User info\n`{P}serverinfo` \u2014 Server info\n`{P}mc` \u2014 Member count\n`{P}accage <id/@u>` \u2014 Account age\n`{P}avatar [@u]` \u2014 Avatar\n`{P}ping` \u2014 Latency\n`{P}botinfo` \u2014 Bot features",
            inline=False)
        e.add_field(name="\U0001f381  Giveaway",
            value=f"`{P}gstart <time> <winners> <prize>` \u2014 Start giveaway\n`{P}gend <msg_id>` \u2014 End giveaway early\n`{P}greroll <msg_id>` \u2014 Reroll winner",
            inline=False)
        e.add_field(name="\u23f2\ufe0f  Timer",
            value=f"`{P}timer <time> <name>` \u2014 Set a timer\nExample: `{P}timer 1h Study Break`",
            inline=False)
        e.set_footer(text=f"StrengthCloud Bot \u2022 {len(bot.guilds)} server(s) \u2022 Prefix: $ • Made by Black Belt")
        await ctx.send(embed=e)

    elif category.lower() in ("ticket", "tickets", "t"):
        e = discord.Embed(title="\U0001f3ab Ticket Commands", color=C_TICKET, description="Manage support tickets")
        cmds = [
            ("$ticket panel",            "Send ticket panel in current channel *(Admin)*"),
            ("$ticket setup",            "Customize panel title & description *(Admin)*"),
            ("$ticket setrules a|b|c",   "Set panel rules separated by `|` *(Admin)*"),
            ("$ticket sethours <time>",  "Set support hours on panel *(Admin)*"),
            ("$ticket close",            "Close & delete the current ticket"),
            ("$ticket add @user",        "Add a user to see this ticket"),
            ("$ticket remove @user",     "Remove a user from this ticket"),
            ("$ticket rename <name>",    "Rename the ticket channel"),
            ("$ticket claim",            "Claim this ticket as staff"),
        ]
        for cmd, desc in cmds:
            e.add_field(name=f"`{cmd}`", value=desc, inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("mod", "moderation", "m"):
        e = discord.Embed(title="\U0001f6e1\ufe0f Moderation Commands", color=C_ERROR, description="Requires appropriate permissions")
        cmds = [
            ("$timeout @user 10m [reason]", "Timeout \u2014 durations: 10s 5m 2h 1d 1w (max 28d)"),
            ("$untimeout @user",             "Remove a member's timeout"),
            ("$kick @user [reason]",         "Kick a member from the server"),
            ("$ban @user [reason]",          "Ban a member from the server"),
            ("$unban <user_id>",             "Unban a user by their ID"),
            ("$warn @user <reason>",         "Warn member \u2014 sends them a DM"),
            ("$warnings @user",              "View all warnings for a member"),
            ("$clearwarns @user",            "Clear all warnings *(Admin)*"),
            ("$purge <1-100> [@user]",       "Bulk delete messages (optional: by user)"),
            ("$lock [#channel]",             "Lock channel \u2014 blocks @everyone sending"),
            ("$unlock [#channel]",           "Unlock a locked channel"),
            ("$slowmode <sec> [#ch]",        "Set slowmode 0 to disable, max 21600"),
            ("$note @user [text]",           "Add or view private staff notes on user"),
        ]
        for cmd, desc in cmds:
            e.add_field(name=f"`{cmd}`", value=desc, inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("afk", "a"):
        e = discord.Embed(title="\U0001f4a4 AFK System", color=C_AFK)
        e.add_field(name="`$afk [reason]`",
            value="Set yourself as AFK\n\u2022 Adds `[AFK]` prefix to your nickname\n\u2022 Auto-replies when someone mentions you\n\u2022 Auto-removes when you send any message",
            inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("invite", "invites", "inv", "i"):
        e = discord.Embed(title="\U0001f4e8 Invite Tracking", color=C_INVITE)
        cmds = [
            ("$i [@user]",            "Detailed invite stats \u2014 joins/left/fake/rejoins/real"),
            ("$lb",                   "Paginated invite leaderboard \u2014 top inviters"),
            ("$resetinvites @user",   "Reset invite count for a user *(Admin)*"),
        ]
        for cmd, desc in cmds:
            e.add_field(name=f"`{cmd}`", value=desc, inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("util", "utility", "u"):
        e = discord.Embed(title="\U0001f527 Utility Commands", color=C_INFO)
        cmds = [
            ...
            ("$botinfo",          "View all bot features"),
        ]
        for cmd, desc in cmds:
            e.add_field(name=f"`{cmd}`", value=desc, inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("giveaway", "gstart", "g"):
        e = discord.Embed(title="\U0001f381 Giveaway Commands", color=discord.Color.gold())
        cmds = [
            ("$gstart <time> <winners> <prize>", "Start a giveaway — e.g. `$gstart 1h 2 Nitro`"),
            ("$gend <message_id>",               "End a giveaway early and pick winners"),
            ("$greroll <message_id>",            "Reroll a new winner for ended giveaway"),
        ]
        for cmd, desc in cmds:
            e.add_field(name=f"`{cmd}`", value=desc, inline=False)
        await ctx.send(embed=e)

    elif category.lower() in ("timer", "t"):
        e = discord.Embed(title="\u23f2\ufe0f Timer Commands", color=discord.Color.orange())
        e.add_field(name="`$timer <duration> <name>`",
            value="Set a timer\n\u2022 Duration: `10s` `5m` `2h` `1d`\n\u2022 Example: `$tstart 30m Study Break`\n\u2022 Bot will ping you when it ends",
            inline=False)
        await ctx.send(embed=e)

    else:
        await ctx.send(embed=err_embed(
            "Unknown category!\n\nAvailable: `ticket` `mod` `afk` `invite` `util` `giveaway` `timer`\nExample: `$help mod`"
        ))
...
async def tp_close(ctx):
    ...
    if TICKET_LOG_ID:
        lch = ctx.guild.get_channel(TICKET_LOG_ID)
        if lch:
            le = discord.Embed(title="🔒 Ticket Closed", color=C_ERROR, timestamp=datetime.now(timezone.utc))
            le.add_field(name="Channel",   value=ctx.channel.name, inline=True)
...
async def tp_rename(ctx, *, name: str):
    clean = re.sub(r"\s+", "-", name.lower())
    await ctx.channel.edit(name=f"ticket-{clean}")
    await ctx.send(embed=ok_embed(f"Renamed to `ticket-{clean}`."))
...
@commands.has_permissions(administrator=True)
async def tp_setping(ctx, role: discord.Role = None):
    global TICKET_PING_ROLE_ID
    if not role:
        TICKET_PING_ROLE_ID = 0
        return await ctx.send(embed=ok_embed("Ping role removed. No role will be pinged on ticket create."))
    TICKET_PING_ROLE_ID = role.id
    await ctx.send(embed=ok_embed(f"Ping role set to {role.mention}\nThis role will be pinged when a new ticket is created."))
